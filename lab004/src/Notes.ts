...
class Notes {
  ...
  private renderNotes(): void {
    const notesContainer = this.container.querySelector(
      ".notes-box:not(.pinned)"
    );
    const pinnedNotesContainer =
      this.container.querySelector(".notes-box.pinned");
    notesContainer.innerHTML = "";
    pinnedNotesContainer.innerHTML = "";

    this.notes.forEach((note: Note, key: number) => {
      let node = note.createNote() as HTMLElement;
  ...
  private registerEditAction(element: HTMLElement, note: Note): void {
    ...
    saveButton.addEventListener("click", () => {
      const inputTitle = element.querySelector(
        '[name="title"]'
      ) as HTMLInputElement;
      const inputMessage = element.querySelector(
        '[name="message"]'
  ...
  private registerColorpickAction(element: HTMLElement, note: Note): void {
    ...
    colorButton.addEventListener("click", () => {
      this.colorpicker.show(colorButton, (color: string) =>
        this.onNoteColorSelection(note, color)
      );
    });
  }
